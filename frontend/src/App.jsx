...
import { SingleLinePageGuide } from "./components/guide/SingleLinePageGuide";
...
function Deploy({ twin, requestConfirm, notify, onAI }) {
    ...
    if (state === "failed") {
      fetch(`${API_URL}/api/analysis/deployment/diagnose`, {
        ...
        body: JSON.stringify({
          ...
          repositoryTwin: twin,
          environment: [],
          requiredEnvironment: twin.environment || []
        })
      })
  ...
  }, [state, twin, API_URL, sampleLog]);

  const configure = () => requestConfirm({ title: `Resolve ${diagnosis?.rootCause || deploymentFinding?.problem || "deployment configuration"}?`, description: "This records approval for the preview configuration change. Project Twin never reads or displays protected values.", action: "Approve configuration" }, () => { setState("configured"); notify("Environment configuration approved"); });
  const retry = () => {
    setState("building");
    setBuildProgress(14);
